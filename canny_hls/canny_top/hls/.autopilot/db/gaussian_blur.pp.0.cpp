...
void gaussian_blur_reset() {
    ...
    VITIS_LOOP_47_1: for (int row = 0; row < KERNEL_SIZE; ++row) {
        VITIS_LOOP_48_2: for (int column = 0; column < WIDTH; ++column) {
...
#pragma HLS ARRAY_PARTITION variable=window type=complete
    ...
    VITIS_LOOP_71_1: for (int tap = 0; tap < KERNEL_SIZE; ++tap) {
#pragma HLS UNROLL
        window[tap] = input[reflect_101(tap - 2, WIDTH)];
    }

    VITIS_LOOP_76_2: for (int column = 0; column < WIDTH; ++column) {
#pragma HLS PIPELINE II=1

        int horizontalSum = 0;

        VITIS_LOOP_81_3: for (int tap = 0; tap < KERNEL_SIZE; ++tap) {
#pragma HLS UNROLL
            horizontalSum +=
        ...
        VITIS_LOOP_90_4: for (int tap = 0; tap < KERNEL_SIZE - 1; ++tap) {
#pragma HLS UNROLL
...
#pragma HLS ARRAY_PARTITION variable=slot type=complete
    ...
    VITIS_LOOP_116_5: for (int tap = 0; tap < KERNEL_SIZE; ++tap) {
#pragma HLS UNROLL
        slot[tap] =
            positive_modulo(
    ...
    VITIS_LOOP_125_6: for (int column = 0; column < WIDTH; ++column) {
#pragma HLS PIPELINE II=1
...
#pragma HLS ARRAY_PARTITION variable=banked type=complete
        ...
        VITIS_LOOP_136_7: for (int bank = 0; bank < KERNEL_SIZE; ++bank) {
#pragma HLS UNROLL
        ...
        VITIS_LOOP_143_8: for (int tap = 0; tap < KERNEL_SIZE; ++tap) {
#pragma HLS UNROLL
            sum +=
